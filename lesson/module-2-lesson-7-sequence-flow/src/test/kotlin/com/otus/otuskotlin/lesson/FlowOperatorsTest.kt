...
class FlowOperatorsTest {
    ...
    fun simple(): Unit = runBlocking {
        ...
        val aaa: Set<Int> = flowOf(1, 2, 3).toSet()
    }
...
//        }
        flowOf(1, 2, 3, 4, 5)
            ...
            .map {
                println("Mapped: $it ${System.currentTimeMillis() - timeInit}ms (${Thread.currentThread().name})")
                it
            }
...
//            .buffer(3, BufferOverflow.DROP_OLDEST) // Попробуйте разные варианты типов и размеров буферов
            ...
            .collect {
                println("Sleep for $it ${System.currentTimeMillis() - timeInit}ms (${Thread.currentThread().name})")
                sleepIndex++
                delay(2_000)
            }
    }
